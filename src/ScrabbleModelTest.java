...
public class ScrabbleModelTest {
    ...
    public void testAIPlayerConstructor() {
        AIPlayer ai = new AIPlayer(0, "TestAI", 1);

        assertEquals("TestAI", ai.getName());
        assertEquals(0, ai.getScore());
        assertEquals(1, ai.getIndex());
        assertNotNull(ai.getTiles());
    }
    ...
    public void testPlaceFirstWord(){
        ...
        player1.getTiles().clear();
        player1.addTile(new Tile('H'));
        player1.addTile(new Tile('E'));
        player1.addTile(new Tile('L'));
        player1.addTile(new Tile('L'));
        player1.addTile(new Tile('O'));

        int score = model.placeWord(7, 7, 1, "HELLO");
        assertEquals("If first word is hello, should be worth 18 (9*2 since first turn)", 18, score);
    }

    @Test
    public void testPlaceDoubleLetterWord(){
        model.initializeGame(2, playerNames);
        Player player1 = model.getCurrentPlayer();
        player1.getTiles().clear();
        player1.addTile(new Tile('H'));
        ...
        player1.addTile(new Tile('L'));
        player1.addTile(new Tile('O'));
        int score1 = model.placeWord(7, 7, 1, "HELLO");
        Player player2 = model.getCurrentPlayer();
        player2.getTiles().clear();
        player2.addTile(new Tile('H'));
        player2.addTile(new Tile('E'));
        player2.addTile(new Tile('L'));
        player2.addTile(new Tile('L'));
        player2.addTile(new Tile('O'));
        int score2 = model.placeWord(6, 8, 2, "HELLO");
